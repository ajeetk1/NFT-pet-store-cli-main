...
contract Store {
  ...
  function nftSale(uint256 _tokenId, uint256 price) external {
    ...
    require(msg.sender == tokenOwners[_tokenId], "NFT");
  ...
  function nftMintBuy(uint256 price, string memory tokenURI) external {
    ...
    require(msg.sender == NFT.owner(), "call nftMintBuy");
    ...
    require(token.balanceOf(msg.sender) >= price, "Insufficient");
  ...
  function nftBuy(uint256 tokenId) public {
    ...
    require(tokenIdForSale[tokenId] == true, "NFT");
    ...
    require(token.allowance(msg.sender, address(this)) >= tokenSalePrice, " allowance");
    require(token.balanceOf(msg.sender) >= tokenSalePrice, "Insufficient ");
    ...
    // TODO: Set that the token is no longer for sale.
    tokenIdForSale[tokenId] = false; //Bool 
